...
def price_card(base: Image.Image, y: int, de: str, por: str) -> int:
    d = ImageDraw.Draw(base)
    _, txt = R.fonts(60, 54)
    bb = d.textbbox((0, 0), f"de {de}", font=txt)
    d.text(((R.W - (bb[2] - bb[0])) / 2, y), f"de {de}", font=txt, fill=R.MUTED)
    y += (bb[3] - bb[1]) + 6
    y = R.draw_display(base, R.W // 2, y, por, 120, accent=None)
    return y
...
def scene_png(offer: dict, kind: str, idx: int, outdir: Path) -> tuple[Path, str, str]:
    ...
    if kind == "flash":
        if idx == 0:
            ...
            handle_footer(bg)
            nar = f"{offer['hook']} {name}, com {disc} por cento de desconto!"
            cap = f"🔥 -{disc}% HOJE"
        elif idx == 1:
            bg = base_scene(12)
            ...
            y = price_card(bg, y + 830, offer["original_label"], offer["price_label"])
            handle_footer(bg)
        ...
        if idx == 0:
            ...
            y = price_card(bg, y + 20, offer["original_label"], offer["price_label"])
...
def build(offer: dict, kind: str, outdir: Path) -> Path:
    ...
    scenes = []
    for i in range(3):
        png, nar, cap = scene_png(offer, kind, i, outdir)
